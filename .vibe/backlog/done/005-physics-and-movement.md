...
**Gate lifted (2026-08-16):** `roadmap`'s `.vibe/backlog/008` ran its Go-vs-Rust spike (see `engine/benchmarks/wasm-lang-spike/`) before this item started, as intended — Product Owner decided to stay on Go. This item proceeds in Go, no rewrite pending. (Native packaging itself moved Tauri → Wails → dropped entirely in favor of a from-scratch native Go UI — roadmap `.vibe/decisions/020`/`021`/`022` — irrelevant to this item's own implementation either way, noted only so none of that superseded phrasing is read as still current.)
