...
## Acceptance Criteria
- [x] `tick.go`'s `fs`, `p1Fs`, `p2Fs` are renamed to `fighter`, `p1Fighter`, `p2Fighter`
- [x] `cmd/wasm/main.go`'s `argString` and `envelope` are renamed to verb-based names (e.g. `extractArgString`, `buildEnvelope`)
- [x] `benchmarks/wasm-lang-spike/go/main.go`'s `hbIdx` is renamed to `hitboxIdx`, and `v0`/`v1` to intent-revealing names (`firstVarIdx`/`secondVarIdx`)
- [x] All existing tests pass unchanged (pure rename, no behavior change)
