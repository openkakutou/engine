...
## Acceptance Criteria
- [x] A `MatchState` (or equivalently named) struct models round/timer and holds two fighters' state
- [x] A per-fighter state type models position, facing, and the identifying fields later items (evaluator, physics, state machine) will need to reference
- [x] Zero-value structs are valid and usable (e.g. a zero-value fighter state doesn't panic when read)
- [x] Invalid construction (e.g. a match state built with fewer than two fighters, if the API allows constructing one) is handled explicitly rather than left to panic downstream
- [x] Unit tests cover the zero-value case and basic field access for both the fighter and match-level types
