...
## Acceptance Criteria
- [x] `tick.go`'s `FighterProgram` gains a precomputed `map[int]air.Animation` (or equivalent), built once, and `tickFighter` looks up by `Anim` number directly instead of scanning
- [x] `zssexec`'s Statedef/Function blocks are indexed once (e.g. by state number / function name) instead of rescanned on every `findStatedef`/`findFunction` call
- [x] All existing tests pass unchanged; no observable behavior change
...
Implemented via `NewFighterProgram` (root package) and `zssexec.Compile`, both precomputing a lookup map once with first-occurrence-wins semantics on duplicate keys. See `.vibe/decisions/013`.
