...
## Acceptance Criteria
- [ ] Expression strings for comparisons and arithmetic/boolean operators parse into an evaluable representation, honoring MUGEN operator precedence
- [ ] Evaluation against a `MatchState`/fighter state fixture returns the correct value for each supported operator and at least the built-in functions/variables listed above
- [ ] A malformed or unparseable expression string returns a descriptive error rather than panicking
- [ ] A well-formed expression referencing an unsupported/unknown trigger name returns a descriptive error rather than silently evaluating to a default
- [ ] Fixture-driven tests evaluate real trigger strings sourced from actual MUGEN/Ikemen GO `.cns` files, not only hand-written expressions
