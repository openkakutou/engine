...
- **`NewFighterProgram(states, animations, commands) FighterProgram`** — the preferred constructor: precomputes `Animations` into an unexported `map[int]air.Animation` once, so `tickFighter`'s per-tick current-animation lookup is O(1) instead of an O(n) scan repeated every tick for the whole match. First-occurrence-wins on a duplicate `Number`, matching the scan it replaces. A `FighterProgram` built via a raw struct literal or JSON-decoded directly (as `cmd/wasm`'s incoming request shape does) has no precomputed index and transparently falls back to the original scan — no existing construction path needs to change. See `.vibe/decisions/013`.
...
- `FighterState` — one fighter's position, facing, velocity, current state number (`StateNo`, matching a loaded character's `cns.StateDef.Number`), health, and power/meter
- `MatchState` — the round number, round timer, and both fighters' `FighterState`, indexed by `Side`
- `NewMatchState(round, roundTimer int, fighters ...FighterState) (*MatchState, error)` — the only way to build a fully validated `MatchState`; it requires exactly one `FighterState` per `Side` and returns a descriptive error otherwise. It also forces each built fighter's power/meter to 0 regardless of what the given `FighterState` carried — the one mechanism giving both match start and every round reset (`round.ResetRound` calls this same constructor) a power/meter that always starts empty, unlike health, which is passed through unchanged. See `.vibe/decisions/015`.
...
- **Implemented controller types** — `ChangeState` (updates `StateNo` to the `value` parameter's evaluated target, validated against a state-existence check, and resets `Time` to 0 — see `.vibe/decisions/004` for why `Step` does not also auto-increment `Time` on every call), `VarSet` (evaluates the `v` and `value` parameters and assigns `Context.Vars[v] = value`), and `PowerAdd` (evaluates its `value` parameter, adds it to `Context.Power`, and clamps the result to `[0, DefaultMaxPower]` — `DefaultMaxPower` is a hardcoded `3000`, MUGEN's own default cap, since `engine` has no lifebar/system-config data to override it with yet; see `.vibe/decisions/015`). Matching real MUGEN/Ikemen behavior, `Step` stops evaluating further controllers as soon as a `ChangeState` applies. A controller of any other type is recorded in `Result.Applied` when its trigger evaluates true, but has no effect — full MUGEN controller-type coverage is expected to grow via later items, not this one.
...
- **`ApplyController(ctrl cns.Controller, ctx *evaluator.Context, exists func(int) bool) (bool, error)`** — the single-controller effect logic `Step` itself uses, exported so `engine/zssexec` can apply the exact same `ChangeState`/`VarSet`/`PowerAdd` behavior to a `.zss` controller-shaped statement instead of re-implementing it. `exists` replaces `Step`'s own `states map[int]cns.StateDef` lookup with a plain predicate, since `zssexec` has no `cns.StateDef` map to check a `.zss` script's own state numbers against. See `.vibe/decisions/008`.
- **`RegisterController(controllerType string, handler ControllerHandler)`** — registers a non-`ChangeState` controller type's effect logic (`ApplyController` dispatches every type except `ChangeState` through this registry, populated at package init time for `VarSet`/`PowerAdd`); adding a new controller type is a `RegisterController` call, not a new `switch` case. `ChangeState` stays a direct special case since it alone needs the state-existence predicate and controls `Step`'s loop termination. `ControllerHandler` takes and returns `evaluator.Context` **by value**, not by pointer — a pointer flowing through the registry's indirect call was measured to defeat escape analysis and force a per-tick heap allocation a direct `switch` call did not. See `.vibe/decisions/016`.
...
- **`Compile(script zss.Script) CompiledScript`** — precomputes a script's `Statedef`/`Function` blocks into two lookup maps (by state number, by function name) once, rather than `Step` rescanning `script.Blocks` on every call for the whole match. First-occurrence-wins on a duplicate state number or function name, matching the linear scan it replaces. See `.vibe/decisions/013`.
- **`Step(ctx evaluator.Context, prog CompiledScript) (Result, error)`** — the package's entry point, mirroring `statemachine.Step`'s shape; takes the already-`Compile`d form rather than a raw `zss.Script`. Finds `prog`'s `Statedef` block for `ctx.StateNo`, parses its body into statements, and runs them from the top against a working copy of `ctx`.
