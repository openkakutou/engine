...
## Acceptance Criteria
- [ ] `evaluator.parsePrimary` is split so its identifier/call-argument-list branch lives in its own helper function, reducing its cyclomatic complexity
- [ ] `evaluator.lex` is split into `scanString`/`scanNumber`/`scanIdent` helpers, reducing its complexity and nesting depth
- [ ] `tick.Tick`'s `bounds`/`gravity`/`tick`/`comboWindow` parameters are grouped into a single config struct, cutting its parameter count
- [ ] All existing tests in `evaluator` and the root package pass unchanged (behavior-preserving refactor)
