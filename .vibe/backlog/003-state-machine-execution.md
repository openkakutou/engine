...
## Acceptance Criteria
- [ ] Given a `StateDef` and a `MatchState`, controllers whose triggers evaluate true are applied and controllers whose triggers evaluate false are skipped, in declared order
- [ ] A state change controller transitions the fighter's current state number and resets per-state execution position correctly
- [ ] Running the loop against a real character's `.cns` idle state produces the expected controller applications for a hand-built `MatchState` fixture
- [ ] A `StateDef`/`Controller` referencing a state number that doesn't exist in the loaded character returns a descriptive error instead of crashing or silently no-op'ing
- [ ] Unit tests cover at least one multi-controller state where trigger order determines which controllers actually apply
